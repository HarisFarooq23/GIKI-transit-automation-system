...
void loadFromFile() {
    ...
    while (fin >> reg) {
        ...
        getline(fin, name); // read full line as name
        studentRoot = insert(studentRoot, reg, name);
    }
    fin.close();
}
...
while (temp != NULL) {
    if (temp->studentID == studentID) {
        cout << "========================================\n";
        cout << setw(18) << left << "Request ID:" << temp->requestID << "\n";
        cout << setw(18) << left << "From:" << campusGraph->getStopName(temp->pickupStop)
             << " (Stop " << temp->pickupStop << ")\n";
        cout << setw(18) << left << "To:" << campusGraph->getStopName(temp->dropStop)
             << " (Stop " << temp->dropStop << ")\n";
        cout << setw(18) << left << "Status:" << "Waiting for shuttle assignment\n";
        cout << "========================================\n\n";
        foundPending = true;
    }
    temp = temp->next;
...
if (!foundPending) {
    cout << "No pending requests for this student.\n";
}
...
while (ptemp != NULL) {
    if (ptemp->studentID == studentID) {
        cout << "========================================\n";
        cout << setw(20) << left << "Request ID:" << ptemp->requestID << "\n";
        cout << setw(20) << left << "Shuttle ID:" << ptemp->shuttleID << "\n";
        cout << setw(20) << left << "From:" 
             << campusGraph->getStopName(ptemp->pickupStop) 
             << " (Stop " << ptemp->pickupStop << ")\n";
        cout << setw(20) << left << "To:" 
             << campusGraph->getStopName(ptemp->dropStop) 
             << " (Stop " << ptemp->dropStop << ")\n";
        cout << setw(20) << left << "Total Distance:" << ptemp->totalDistance << " units\n";
        cout << setw(20) << left << "Estimated Time:" << ptemp->estimatedTime << " minutes\n";
        cout << setw(20) << left << "Status:" << ptemp->status << "\n";
        cout << "========================================\n\n";
        foundProcessed = true;
    }
    ptemp = ptemp->next;
...
if (!foundProcessed) {
    cout << "No recent rides for this student.\n";
}
...
if (pr == NULL) {
    ...
    while (pr != NULL) {
        cout << setw(9) << left << pr->requestID << " | "
             << setw(9) << left << pr->studentID << " | "
             << setw(9) << left << pr->shuttleID << " | "
             << setw(16) << left << campusGraph->getStopName(pr->pickupStop) << " | "
             << setw(16) << left << campusGraph->getStopName(pr->dropStop) << " | "
             << setw(8) << left << pr->totalDistance << " | "
             << pr->status << "\n";
        pr = pr->next;
    }
}
...
if (cr == NULL) {
    ...
    while (cr != NULL) {
        cout << cr->requestID << "         | "
             << cr->studentID << "         | "
             << setw(16) << left << campusGraph->getStopName(cr->pickupStop) << " | "
             << setw(16) << left << campusGraph->getStopName(cr->dropStop) << "   | "
             << setw(13) << left << cr->timeRequested << " | "
             << cr->priority << "\n";
        cr = cr->next;
    }
}
    ...
    do {
        ...
        switch (choice) {
            ...
            case 4:
                ...
                cout << "| Pending Requests       | " << setw(26) << left 
                     << (requestFront != NULL ? "Yes" : "None") << "|\n";
                cout << "------------------------------------------------------\n";
